 ...
 * 基于注解的监听消息消费者
 */
@RocketMQMessageListener(topic = "${rocketmq.name.async.topic}", consumerGroup = "${rocketmq.name.consumer.async.group.first}")
@Component
public class FirstASyncConsumerService implements RocketMQListener<MessageExt> {
    ...
    public void onMessage(MessageExt messageExt) {
        System.out.println("TOPIC:kongquan-async-test 消费者rocketmq.name.consumer.async.group.first收到消息：" + JSON.parseObject(messageExt.getBody(), User.class));
        try {
            // TODO 处理业务逻辑
            // 修改消息消费状态 成功

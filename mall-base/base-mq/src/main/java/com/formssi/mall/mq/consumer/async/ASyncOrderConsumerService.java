 ...
 * 基于注解的监听消息消费者 带顺序消费
 */
@RocketMQMessageListener(topic = "${rocketmq.name.async.topic.order}", consumerGroup = "kongquan-async-order-test-group",consumeMode = ConsumeMode.ORDERLY)
@Component
public class ASyncOrderConsumerService implements RocketMQListener<MessageExt> {
    ...
    public void onMessage(MessageExt messageExt) {
        System.out.println("ASyncOrderConsumerService收到消息,带顺序消费：" + JSON.parseObject(messageExt.getBody(), User.class));
        try {
            // TODO 处理业务逻辑
            // 修改消息消费状态 成功

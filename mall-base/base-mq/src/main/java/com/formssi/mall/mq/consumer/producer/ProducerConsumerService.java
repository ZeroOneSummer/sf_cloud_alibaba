 ...
 * 基于注解的监听消息消费者
 */
@RocketMQMessageListener(topic = "${rocketmq.name.producer.topic}", consumerGroup = "${rocketmq.name.consumer.producer.group}")
@Component
public class ProducerConsumerService implements RocketMQListener<MessageExt> {
    @Autowired
    private MqLogSuccessMapper mqLogSuccessMapper;

    @Override
    public void onMessage(MessageExt messageExt) {
        System.out.println("TOPIC:kongquan-producer-test 消费者rocketmq.name.consumer.producer.group收到消息" + JSON.parseObject(messageExt.getBody(), User.class));
        try {
            // TODO 处理业务逻辑
            // 修改消息消费状态 成功

...
import com.formssi.mall.common.util.NumberSerialUtil;
import com.formssi.mall.common.util.NumberTypeEnum;
...
public class RedisService {
    ...
    public Long getIncrementNum(String key) {
        ...
        // 初始设置过期时间,过了当天就清零重新累加，避免过多的key
        if ((null == counter || counter.longValue() == 1)) {
            System.out.println("自增长ID设置过期时间为1天!,key="+key);
            // 设置清除的目的：让每天的计数器都从0开始
     ...
     * @return
     */
    public String getDistributedId(NumberTypeEnum numberTypeEnum){
        //获得单号前缀
        //格式 固定前缀 +时间前缀 示例 ：SP22041814
        String formNoPrefix = NumberSerialUtil.getFormNoPrefix(numberTypeEnum);
        //获得缓存key
        String cacheKey = NumberSerialUtil.getCacheKey(formNoPrefix);
        //获取redis自增长序号
        Long incrementalSerial = getIncrementNum(cacheKey);
        //组合单号并补全流水号
        String serialWithPrefix = NumberSerialUtil
                .completionSerial(formNoPrefix, incrementalSerial, numberTypeEnum);
        //补全随机数
        return NumberSerialUtil.completionRandom(serialWithPrefix, numberTypeEnum);
    }
}
